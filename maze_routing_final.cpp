...
{
	int x = -55;							//the x coordiante of the pin, -55 to mark uninitialized pins
	int y;									//the y coordinate of the pin
	int layer;								//the layer in which the pin resides 
...
vector<vector<pin>> getting_input(string filename)								//the main function for extracting the input and sorting the nets ascendingly to be handeled 
{
	...
	sort(vect.begin(), vect.end(), sortbysec);					//sorting each net by the perimeter of the box that contains the pins
	vector<vector<pin>> returned_nets;
	for (int i = 0; i < net.size(); i++)
		returned_nets.push_back(net[vect[i].first]);

	return returned_nets;
}
...
void maze_routing(vector<vector<pin>> nets)
{
	...
	output_nets.resize(number_nets);
	...
	int* attempted_ripups; // number of attempted rip ups for each wire, to end the program if a wire gets ripped up too many times
	attempted_ripups = new int[number_nets];
	for (int i = 0; i < number_nets; i++)
		attempted_ripups[i] = 1;

	for (int i = 0; i < number_nets; i++)
	{
		// Change the order of pins in case of rerouting
		if (attempted_ripups[i] > 1)
		{
			int temp_pin_index = rand() % nets[i].size();
			pin temp_pin = nets[i][temp_pin_index];
			nets[i][temp_pin_index] = nets[i].back();
			nets[i].pop_back();
			nets[i].push_back(temp_pin);
		}
		vector<pin>* targets;
		...
		for (int j = 0; j < nets[i].size() - 1; j++)
		{
			...
			do
			{
				...
				switch (cur_layer) {
				case 0:
					if (cells[1][cur_x][cur_y] == 0 || (cells[1][cur_x][cur_y] > cur_num + 10 && cells[1][cur_x][cur_y] != 1000000000))
					{
				...
				case NUMBER_LAYERS - 1:
					if (cells[NUMBER_LAYERS - 2][cur_x][cur_y] == 0 || (cells[NUMBER_LAYERS - 2][cur_x][cur_y] > cur_num + 10 && cells[NUMBER_LAYERS - 2][cur_x][cur_y] != 1000000000))
					{
					...
					break;
				default:
					if (cells[cur_layer - 1][cur_x][cur_y] == 0 || (cells[cur_layer - 1][cur_x][cur_y] > cur_num + 10 && cells[cur_layer - 1][cur_x][cur_y] != 1000000000))
					{
				...
				switch (cur_x) {
				case 0:
					if (cells[cur_layer][1][cur_y] == 0 || (cells[cur_layer][1][cur_y] > cur_num + 10 && cells[cur_layer][1][cur_y] != 1000000000))
					{
					...
					break;
				case N - 1:
					if (cells[cur_layer][N - 2][cur_y] == 0 || (cells[cur_layer][N - 2][cur_y] > cur_num + 10 && cells[cur_layer][N - 2][cur_y] != 1000000000))
					{
					...
					break;
				default:
					if (cells[cur_layer][cur_x + 1][cur_y] == 0 || (cells[cur_layer][cur_x + 1][cur_y] > cur_num + 10 && cells[cur_layer][cur_x + 1][cur_y] != 1000000000))
					{
				...
				switch (cur_y) {
				case 0:
					if (cells[cur_layer][cur_x][1] == 0 || (cells[cur_layer][cur_x][1] > cur_num + 10 && cells[cur_layer][cur_x][1] != 1000000000))
					{
					...
					break;
				case N - 1:
					if (cells[cur_layer][cur_x][N - 2] == 0 || (cells[cur_layer][cur_x][N - 2] > cur_num + 10 && cells[cur_layer][cur_x][N - 2] != 1000000000))
					{
					...
					break;
				default:
					if (cells[cur_layer][cur_x][cur_y + 1] == 0 || (cells[cur_layer][cur_x][cur_y + 1] > cur_num + 10 && cells[cur_layer][cur_x][cur_y + 1] != 1000000000))
					{
				...
				for (int k = 0; k < targets->size(); k++)
					...
					{
						reached_target = true;
						if (found_target.x == -55 || cells[found_target.layer][found_target.x][found_target.y] > cells[targets->at(k).layer][targets->at(k).x][targets->at(k).y])
			...
			{
				int m;
				if (attempted_ripups[i] > i)
				{
					cout << "ERROR!!!\nRip-Up failed!!!\n";
					return;
				}
				for (m = 1; m <= attempted_ripups[i]; m++)
				{
			...
			while (cur_sol_num != -1)
			{
				...
				switch (cur_layer) {
				case 0:
					if ((cells[1][cur_x][cur_y] + 10 == cur_sol_num || cells[1][cur_x][cur_y] + 10 + 1 == cur_sol_num && cells[1][cur_x][cur_y] == -1) && cells[1][cur_x][cur_y] != 0 && !next_cell_found)
					{
					...
					break;
				case NUMBER_LAYERS - 1:
					if ((cells[NUMBER_LAYERS - 2][cur_x][cur_y] + 10 == cur_sol_num || cells[NUMBER_LAYERS - 2][cur_x][cur_y] + 10 + 1 == cur_sol_num && cells[NUMBER_LAYERS - 2][cur_x][cur_y] == -1) && cells[NUMBER_LAYERS - 2][cur_x][cur_y] != 0 && !next_cell_found)
					{
						pin p_temp = found_target;
					...
					break;
				default:
					if ((cells[cur_layer - 1][cur_x][cur_y] + 10 == cur_sol_num || cells[cur_layer - 1][cur_x][cur_y] + 10 + 1 == cur_sol_num && cells[cur_layer - 1][cur_x][cur_y] == -1) && cells[cur_layer - 1][cur_x][cur_y] != 0 && !next_cell_found)
					{
						pin p_temp = found_target;
						p_temp.layer = found_target.layer - 1;
						targets->push_back(p_temp);
				...
				switch (cur_x) {
					case 0:
						if ((cells[cur_layer][1][cur_y] + added_x == cur_sol_num || cells[cur_layer][1][cur_y] + added_x + 1 == cur_sol_num && cells[cur_layer][1][cur_y] == -1) && cells[cur_layer][1][cur_y] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
							p_temp.x = 1;
							targets->push_back(p_temp);
						...
						break;
					case N - 1:
						if ((cells[cur_layer][N - 2][cur_y] + added_x == cur_sol_num || cells[cur_layer][N - 2][cur_y] + added_x + 1 == cur_sol_num && cells[cur_layer][N - 2][cur_y] == -1) && cells[cur_layer][N - 2][cur_y] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
						...
						break;
					default:
						if ((cells[cur_layer][cur_x + 1][cur_y] + added_x == cur_sol_num || cells[cur_layer][cur_x + 1][cur_y] + added_x + 1 == cur_sol_num && cells[cur_layer][cur_x + 1][cur_y] == -1) && cells[cur_layer][cur_x + 1][cur_y] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
							p_temp.x = found_target.x + 1;
							targets->push_back(p_temp);
				...
				switch (cur_y) {
					case 0:
						if ((cells[cur_layer][cur_x][1] + added_y == cur_sol_num || cells[cur_layer][cur_x][1] + added_y + 1 == cur_sol_num && cells[cur_layer][cur_x][1] == -1) && cells[cur_layer][cur_x][1] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
							p_temp.y = 1;
							targets->push_back(p_temp);
						...
						break;
					case N - 1:
						if ((cells[cur_layer][cur_x][N - 2] + added_y == cur_sol_num || cells[cur_layer][cur_x][N - 2] + added_y + 1 == cur_sol_num && cells[cur_layer][cur_x][N - 2] == -1) && cells[cur_layer][cur_x][N - 2] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
						...
						break;
					default:
						if ((cells[cur_layer][cur_x][cur_y + 1] + added_y == cur_sol_num || cells[cur_layer][cur_x][cur_y + 1] + added_y + 1 == cur_sol_num && cells[cur_layer][cur_x][cur_y + 1] == -1) && cells[cur_layer][cur_x][cur_y + 1] != 0 && !next_cell_found)
						{
							pin p_temp = found_target;
							p_temp.y = found_target.y + 1;
							targets->push_back(p_temp);
